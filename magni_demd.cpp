...
int main(){
	Mat var;
	var=imread("/home/subham/Image_Processing/index.png",1);
	Mat var2;
	float magni;
	...
	if(magni>=0){
		...
		for(int i=0;i<var2.rows;i++){
			for(int j=0;j<var2.cols;j++){
				var2.at<Vec3b>(i,j)[0]=var.at<Vec3b>(i/abs(mg),(var2.cols-1-j)/abs(mg))[0];
				var2.at<Vec3b>(i,j)[1]=var.at<Vec3b>(i/abs(mg),(var2.cols-1-j)/abs(mg))[1];
				var2.at<Vec3b>(i,j)[2]=var.at<Vec3b>(i/abs(mg),(var2.cols-1-j)/abs(mg))[2];
			}
		}
	}
	imshow("Display",var2);
